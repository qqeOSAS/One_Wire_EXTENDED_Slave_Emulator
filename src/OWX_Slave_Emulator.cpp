...
void Emulator::writeScratchpad(const uint8_t* data, uint8_t len) {
    if(len > sizeof(scratchpad)) len = sizeof(scratchpad);
    memcpy(scratchpad, data, len);
    scratchpadLen = len; // оновлюємо довжину
}
...
{   
    ...
    switch(low_cmd){
        ...
        case OW_LOW_CMD_SEND_VARIABLE_:
            ...
            uint8_t* payload = (uint8_t*)malloc(len ? len : 1); // allocate at least 1 to avoid null ptr issues
            if (!payload && len) { hub->raiseDeviceError(cmd); return; }

            if (len) {
                if (hub->recv(payload, len)) { free(payload); hub->raiseDeviceError(cmd); return; }
            }

            uint8_t recv_crc;
            if (hub->recv(&recv_crc, 1)) { free(payload); hub->raiseDeviceError(cmd); return; }
            ...
            bool handled = processCommand(cmd, payload, len, hub);
...
bool Emulator::processCommand(uint8_t cmd, const uint8_t *payload, uint8_t len, OneWireHub *hub)
{
    bool handled = false;
    ...
    switch(cmd){
        ...
        case OW_CMD_UINT32:
            if(len != 4) { hub->raiseDeviceError(cmd); return false; }
